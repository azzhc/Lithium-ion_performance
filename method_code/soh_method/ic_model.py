...
class Net(nn.Module):  # 三者都相同，所以直接使用mode进行区别就行
    def __init__(self, input_size,  n_class=160, mode='LSTM'):
        ...
        self.dropout = nn.Dropout(p=0.2)#SOH
...
def model_train(model_select,best_model_path, best_model_name,parameter_path,window_size,
    ...
    parameter.write(str(window_size) + ','+str(num_classes)+','+str(v_mean)+','+str(v_std)+','+model_select)
    parameter.close
'''
    构建模型训练函数
'''
...
# v_data,ic_data,num_classes=ic_getin(data_formate,v_path,ic_path)#这是数据加载部分的结果
# #然后才是模型选择和模型参数设置的变量
